...
let parseSample (sample: string) =
    let rows =
        sample
        |> getLines
        |> Seq.map (fun line -> line.ToCharArray() |> Seq.toList)
        |> Seq.toList
...
let getCombinations (sourceList: char list list) =
    ...
    |> Seq.map (fun data ->
        let rows = data |> Seq.map List.ofSeq |> List.ofSeq

        { Rows = rows
          Columns = rows |> List.transpose })

let describeLine item =
    let horizontal = findMiddle item.Rows |> Seq.map Horizontal
    let vertical = findMiddle item.Columns |> Seq.map Vertical
...
type Solution() =
    interface ISolution with
        ...
        override this.SolvePart1(input) =
            ...
            let result = parsed |> Seq.map (describeLine >> List.exactlyOne) |> Seq.sum
        ...
        override this.SolvePart2(input) =
            ...
            let result =
                parsed
                |> Seq.map (fun line ->
                    let current = line |> describeLine |> List.exactlyOne
