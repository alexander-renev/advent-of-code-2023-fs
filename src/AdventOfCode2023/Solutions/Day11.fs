...
let toArray source =
    source |> Seq.map Seq.toArray |> Seq.toArray
...
let horizontalExpander (count: int64) (point: Point) = Space { point with X = count }

let verticalExpander (count: int64) (point: Point) = Space { point with Y = count }
...
let parseInput (input: string) =
    ...
    |> Seq.map (fun line ->
        line.ToCharArray()
        |> Seq.map (fun ch -> if ch = '#' then Galaxy else Space { X = 1L; Y = 1L })
        |> Seq.toArray)
    |> Seq.toArray
