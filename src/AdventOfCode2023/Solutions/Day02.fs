...
type Card =
    { Number: int
      Combinations: IDictionary<Color, int> list }
...
type Solution() =
    interface ISolution with
        ...
        override this.SolvePart1(input) =
            let sum =
                parseInput input |> Seq.filter suitsForPart1 |> Seq.map _.Number |> Seq.sum

            printfn $"Sum is {sum}"

        override this.SolvePart2(input) =
            let sum = parseInput input |> Seq.map processCardForPart2 |> Seq.sum
            printfn $"Sum is {sum}"
