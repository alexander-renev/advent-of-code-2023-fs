...
let parseInput (input: string) =
    ...
    let source =
        ...
        |> Seq.collect (fun (y, line) -> line |> Seq.mapi (fun x ch -> ({ X = x; Y = y }, ch)))
    ...
    let asterisks =
        source |> Seq.filter (fun p -> snd p = '*') |> Seq.map fst |> Seq.toList
    ...
    interface ISolution with
        ...
        override this.SolvePart2(input) =
            ...
            let gears =
                ...
                |> Seq.map snd
                |> Seq.sumBy (fun nums -> nums |> Seq.map snd |> Seq.reduce (*))
