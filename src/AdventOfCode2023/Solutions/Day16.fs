...
let getEnergizedCount (parsed: Mirrors) (start: Light) =
    ...
    while queue.Count > 0 do
        ...
        if
            visited.ContainsKey(position) = false
            || visited[position] &&& direction = direction
        then
            ()
        else
            ...
            let nextDirections =
                ...
                | '.' -> [ direction ]
                | '|' when direction = Direction.Up || direction = Direction.Down -> [ direction ]
                | '-' when direction = Direction.Left || direction = Direction.Right -> [ direction ]
                | '/' ->
                    match direction with
    ...
    let energized =
        visited.Values |> Seq.filter (fun v -> v <> Direction.None) |> Seq.length
...
type Solution() =
    interface ISolution with
        ...
        override this.SolvePart2(input) =
            ...
            let maxEnergized =
                vertical |> Seq.append horizontal |> Seq.map getEnergized |> Seq.max
