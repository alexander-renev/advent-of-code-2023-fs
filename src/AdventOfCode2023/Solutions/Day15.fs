...
let getHash (text: string) =
    (0, text.ToCharArray())
    ||> Seq.fold (fun state ch -> (state + int (ch)) * 17 % 256)
...
type Solution() =
    interface ISolution with
        ...
        override this.SolvePart1(input) =
            let sum = input |> parseInput1 |> Seq.map getHash |> Seq.sum
            printfn $"Sum is {sum}"

        override this.SolvePart2(input) =
            let parsed = parseInput2 input
            let boxes = seq { 0..255 } |> Seq.map (fun i -> (i, LinkedList<Lens>())) |> dict
            ...
            let lenses =
                boxes.Values |> Seq.collect id |> Seq.map _.Label |> Seq.distinct |> Seq.toList
