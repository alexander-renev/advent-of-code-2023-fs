...
let numberMap =
    digitsMap
    |> Seq.append (seq { 1..9 } |> Seq.map (fun i -> (i.ToString(), i)))
    |> dict
...
type Solution() =
    interface ISolution with
        member this.Input =
            createCustomInput 1 (fun (input: Input) ->
        ...
        member this.SolvePart1(input) =
            let sum = getLines input |> Seq.map getNumber |> Seq.sum
            printfn $"Sum is %i{sum}"

        member this.SolvePart2(input) =
            let sum = getLines input |> Seq.map getNumberOrText |> Seq.sum
            printfn $"Sum is {sum}"
