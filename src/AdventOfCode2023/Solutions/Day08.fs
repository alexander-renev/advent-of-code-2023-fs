...
type Description =
    { Positions: int64 list
      Period: bigint }

let makeInstructions (instructions: Command list) =
    seq {
        while true do
            yield! instructions |> Seq.indexed
    }

let describeInput (start: string) (commands: Command list) (map: IDictionary<String, Node>) =
    let visited = HashSet<_>()
    let mutable position = start
    visited.Add(position, -1) |> ignore
    let result = ResizeArray<_>()

    makeInstructions commands
    |> Seq.mapi (fun index (commandIndex, command) ->
        position <-
            match command with
    ...
    let period =
        duplicates |> Seq.rev |> Seq.map (fun (_, index, _) -> index) |> Seq.reduce (-)
...
let parseInput (input: string) =
    let instructionParser =
        choice [ pstring "R" >>% Command.GoRight; pstring "L" >>% Command.GoLeft ]
    ...
    let nodeParser =
        itemParser .>> pstring " = (" .>>. itemParser .>> pstring ", " .>>. itemParser
        .>> pstring ")"
        |>> (fun ((from, toLeft), toRight) -> (from, { Left = toLeft; Right = toRight }))

    let nodesParser = sepEndBy nodeParser newline |>> dict
    let inputParser = instructionsParser .>> newline .>> newline .>>. nodesParser
...
type Solution() =
    interface ISolution with
        override this.Input =
            createCustomInput 8 (fun (input: Input) ->
        ...
        override this.SolvePart1(input) =
            let commands, nodes = parseInput input
        ...
        override this.SolvePart2(input) =
            ...
            let mutable positions =
                nodes |> Seq.map _.Key |> Seq.filter (fun s -> s.EndsWith "A") |> Seq.toList

            let descriptions =
                positions |> Seq.map (fun pos -> describeInput pos commands nodes) |> Seq.toList
