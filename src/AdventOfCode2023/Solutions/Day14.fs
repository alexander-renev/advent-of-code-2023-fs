...
let getWidth (input: Dictionary<Point, Position>) = input.Keys |> Seq.map _.X |> Seq.max

let getHeight (input: Dictionary<Point, Position>) = input.Keys |> Seq.map _.Y |> Seq.max
    ...
    |> Seq.iter (fun points ->
        ...
        while moved do
            let positionToMove =
                points
                |> Seq.pairwise
                |> Seq.filter (fun (prev, pos) -> input[pos] = Round && input[prev] = Empty)
                |> Seq.tryHead

            positionToMove
            |> Option.iter (fun (prev, pos) ->
                input[prev] <- Round
                input[pos] <- Empty)
...
let moveCycle (input: Dictionary<Point, Position>) =
    input |> move North |> move West |> move South |> move East |> ignore
...
type Solution() =
    interface ISolution with
        ...
        override this.SolvePart1(input) =
            ...
            let result = parsed |> move North |> getWeight
