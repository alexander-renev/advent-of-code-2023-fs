...
type Solution() =
    ...
    member private this.GetLoop(input: Point array array) =
        let startPoint =
            input
            |> Seq.collect id
    ...
    member private this.GetNeighbours (point: Point) =
    ...
    member private this.ParseInput (input: string) =
        ...
        |> Seq.mapi (
            fun y line ->
                line.ToCharArray()
                |> Seq.mapi (fun x ch -> { Coordinates = {X = x; Y = y}; Position = this.GetPosition(ch) })
                |> Seq.toArray
            )
        |> Seq.toArray
    
    member private this.GetPosition(p: char) =
        match p with
        | '.' -> Ground
        | 'S' -> Start
        | '|' -> Pipe BottomTop
