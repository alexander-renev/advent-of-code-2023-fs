...
let getLoop (input: Point array array) =
    let startPoint = input |> Seq.collect id |> Seq.find (fun pt -> pt.Position = Start)
    ...
    let neighbours =
        ...
        |> Seq.filter (fun p ->
            p
            |> getNeighbours
            |> Option.map (fun (first, second) -> first = start || second = start)
            |> Option.defaultValue false)
        |> Seq.toList

    let neighbour = Seq.head neighbours
    let neighbourCoords = neighbours |> Seq.map _.Coordinates |> Set.ofSeq

    let startPosition =
        [ BottomTop; LeftRight; TopLeft; TopRight; BottomLeft; BottomRight ]
        |> Seq.map (fun dir -> { startPoint with Position = Pipe dir })
        |> Seq.choose (fun pt -> pt |> getNeighbours |> Option.map (fun nb -> (pt, nb)))
        |> Seq.filter (fun (_, (first, second)) -> neighbourCoords.Contains(first) && neighbourCoords.Contains(second))
        |> Seq.map fst
        |> Seq.map _.Position
        |> Seq.head
...
type Solution() =
    interface ISolution with
        override this.Input =
            createCustomInput 10 (fun (input: Input) ->
        ...
        override this.SolvePart2(input) =
            ...
            let freePoints =
                ...
                |> Seq.collect id
                |> Set.ofSeq
                |> fun pts -> loop |> Set.ofSeq |> Set.difference pts
            ...
            let maxY = parsed |> Seq.collect id |> Seq.map _.Coordinates.Y |> Seq.max
