...
type InputHelper =
    static member GetResourceAsString(name: string) =
        ...
        let result =
            tp.Assembly.GetManifestResourceStream("AdventOfCode2023.Inputs." + name)

        if isNull result then
            failwith $"resource {name} not found"
...
type Input(day: int) =
    ...
    member this.TestData = this.GetData("test")

    member this.RealData = this.GetData("real")
